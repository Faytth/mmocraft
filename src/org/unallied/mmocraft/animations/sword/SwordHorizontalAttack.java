...
public class SwordHorizontalAttack extends Rollable {
    ...
    public SwordHorizontalAttack(Living player, AnimationState last) {
        super(player, last);
        animation = new Animation();
        animation.setAutoUpdate(false);
        animation.setLooping(false);
        SpriteSheetNode node = SpriteHandler.getInstance().getNode(SpriteID.SWORD_HORIZONTAL_ATTACK.toString());
    ...
    public void update(long delta) {
        ...
        if (elapsedTime > duration) {
            if (living.isShielding()) {
                ...
                living.setState(new SwordHorizontalAttack2(living, this));
            } else {
    ...
    @Override
    public short getId() {
        return AnimationID.SWORD_HORIZONTAL_ATTACK.getValue();
    }
